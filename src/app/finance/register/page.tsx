...
import { Mail, Lock, Eye, EyeOff, User, Loader2, CheckCircle2, TrendingUp } from "lucide-react";
...
export default function FinanceRegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/portal/finance/register", {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      {/* Left panel */}
      <div className="fin-reg-left" style={{
        width: 520, flexDirection: "column", justifyContent: "space-between",
        position: "relative", overflow: "hidden", display: "none",
      }}>
        {/* Background image */}
        <img
          src="https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=1200&q=80&fit=crop"
          alt=""
        ...
        <div style={{ position: "relative", zIndex: 10, padding: "0 40px 24px", flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <h1 style={{ color: "#fff", fontSize: 34, fontWeight: 800, lineHeight: 1.2, marginBottom: 18, letterSpacing: -0.5 }}>
            Join the<br />Finance<br />Portal
          </h1>
          <p style={{ color: "rgba(255,255,255,0.72)", fontSize: 15, lineHeight: 1.8, maxWidth: 340 }}>
            Create your finance team account and get immediate access to payroll management, budget tracking, and financial reports.
          </p>
          <div style={{ marginTop: 32, display: "flex", flexDirection: "column", gap: 12 }}>
            {["Payroll & bulk payments", "Budget tracking", "Financial reports", "Audit logs"].map(f => (
        ...
        <div style={{ position: "relative", zIndex: 10, padding: "0 40px 40px" }}>
          <div style={{ background: "rgba(255,255,255,0.1)", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 12, padding: "14px 16px" }}>
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.08em", margin: "0 0 3px" }}>Finance Portal</p>
            <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 13, fontWeight: 500, margin: 0 }}>Secure access for your finance team</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "var(--bg-body)", padding: "40px 24px" }}>
        <div style={{ width: "100%", maxWidth: 460 }}>
          {/* Mobile brand */}
          <div className="fin-reg-mobile-brand" style={{ marginBottom: 28, textAlign: "center" }}>
          ...
          <div style={{ backgroundColor: "var(--bg-card)", borderRadius: 16, boxShadow: "var(--shadow-card)", padding: "28px 32px", border: "1px solid var(--border)" }}>
            {success ? (
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>Account Created!</h2>
                <p style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 4 }}>Your finance portal account is ready.</p>
                <p style={{ fontSize: 12, color: "var(--text-muted)" }}>Check your email for your credentials. Redirecting…</p>
              </div>
            ) : (
              <>
                <div style={{ marginBottom: 24 }}>
                  <h2 style={{ fontSize: 22, fontWeight: 700, color: "var(--text-primary)", marginBottom: 4 }}>Create Finance Account</h2>
                  <p style={{ color: "var(--text-secondary)", fontSize: 13 }}>Set up your finance portal access</p>
                </div>
                {error && (
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Jane Smith" style={inp} />
                    </div>
                  </div>
                  <div>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      <input type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="finance@company.com" style={inp} />
                    </div>
                  </div>
                  <div>
                  ...
                  <button type="submit" disabled={loading} style={{ width: "100%", padding: "11px 16px", borderRadius: 10, color: "#fff", fontWeight: 600, fontSize: 14, border: "none", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "var(--accent)", marginTop: 4 }}>
                    {loading ? <><Loader2 size={16} style={{ animation: "spin 1s linear infinite" }} /> Creating account…</> : "Create Finance Account"}
                  </button>
                </form>
                <p style={{ marginTop: 20, textAlign: "center", fontSize: 13, color: "var(--text-secondary)" }}>
                  Already have an account?{" "}
                  <Link href="/finance/login" style={{ fontWeight: 600, color: "var(--accent)", textDecoration: "none" }}>Sign in</Link>
                </p>
          ...
          <p style={{ marginTop: 20, textAlign: "center", fontSize: 11, color: "var(--text-muted)" }}>
            NeraAdmin v1.0 — Finance Portal Registration
          </p>
        </div>
      </div>
      ...
      <style>{`
        @media(min-width: 1024px) {
          .fin-reg-left { display: flex !important; }
          .fin-reg-mobile-brand { display: none !important; }
        }
