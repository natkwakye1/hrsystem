...
// Use prisma db execute to seed via SQL instead
import { readFileSync, writeFileSync, existsSync } from "fs";
import { randomBytes } from "crypto";

// We'll use the Prisma CLI to seed via SQL
const dbPath = resolve(projectRoot, "prisma/dev.db");

// Import better-sqlite3 or use prisma db execute
// Let's just use prisma's built-in approach with the generated client
// Since Prisma v7 generates TS, we need tsx to run the seed

console.log("Seeding via prisma db execute...");

const bcryptHash = "$2a$12$LJ/V0E0xYFkY.xVV1n2FqOKA8wBvdYLqCJJKqYW0Ge.W1vVQiVG.i"; // pre-hashed: admin123

const sql = `
-- Admin user
INSERT OR IGNORE INTO User (id, email, password, name, role, createdAt, updatedAt)
VALUES ('admin001', 'admin@hrsystem.com', '${bcryptHash}', 'System Admin', 'admin', datetime('now'), datetime('now'));
...
writeFileSync(resolve(projectRoot, "prisma/seed.sql"), sql);
execSync(`cd "${projectRoot}" && npx prisma db execute --file prisma/seed.sql`, { stdio: "inherit" });
